class Admins::NewsController < AdminBaseController
  ...
  def index
    @admins_news = News.unscoped.eager_load(comments: :user, views: :user)
  ...
  def admins_news_params
    params.require(:news).permit(:content, :tags, :send_push, :poster_avatar, :expiry_date, :release_date, :link, :pdf, :title, :author_name, :send_to_all, position_ids: [], access_level_ids: [],individual_user_ids:[], department_ids: [], practise_code_ids: [], direct_report_ids: [], media_files_attributes: [ :id, :fileable_id, :fileable_type, :file, :_destroy ])
  end
end
